...
class ConfigWrapper:
    ...
    def getDBserverMemory(self, default=sys.maxint):
        DBServerElem = self.xmlRoot.find("DBserver")
        if DBServerElem is not None and "memory" in DBServerElem.attrib:
            return int(DBServerElem.attrib["memory"])
        return default
    
    def getDBserverCpu(self, default=sys.maxint):
        DBServerElem = self.xmlRoot.find("DBserver")
